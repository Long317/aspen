	...
	public boolean parse(String fullPath) {
		...
		try {
			...
			for (int i = 0; i < shows.size(); i++) {
				...
				try {
					synchronized (lock) {
						double[] geoData = AddressConverter
				...
				// if no theatre in db save it
				if (TheatreDAO.getInstance().getTheatreByName(t.getName())
						 == null) {
					...
					t=TheatreDAO.getInstance().getTheatreByName(t.getName());
				}
